...
pH_H2O_CH4_letters <- map_df(names(pH_H2O_CH4_models), function(st) {
  ...
  # Get the fitted model for this soil type
  model <- pH_H2O_CH4_models[[st]]$model
  
  # Compute emmeans afresh
  emm <- emmeans(model, pairwise ~ pH_treatment, adjust = "none", level = 0.90)
  
  # Generate compact letter display
  cld_df <- cld(emm, adjust = "none", Letters = letters, alpha = 0.10, sort = FALSE)
  
  # Prepare for plotting
  cld_df <- as.data.frame(cld_df)
  cld_df$soil_type <- st
  cld_df$letters <- cld_df$.group
  
  cld_df[, c("soil_type", "pH_treatment", "letters")]
}) %>% 
  mutate(
    soil_type    = factor(soil_type, levels = c("Unamended", "Ground Rock")),  
    pH_treatment = factor(pH_treatment, levels = c("Wet control", "5", "7", "8"))  
...
# Plot
pH_H2O_CH4_fluxes_means <- ggplot(summ_exp_letters, 
                                  aes(x = pH_treatment, 
  ...
  geom_text(aes(label = letters,
                y = ng_CH4_per_g_dry_soil_per_day_mean - 1.3*ng_CH4_per_g_dry_soil_per_day_se),
