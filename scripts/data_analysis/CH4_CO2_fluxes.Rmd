...
pH_H2O_CH4_letters <- summ_exp %>%
  mutate(
    letters = c("a", "ab", "bc", "c", "ab", "c", "a", "bc"),   # must match row order!
...
# Plot
pH_H2O_CH4_fluxes_means <- ggplot(summ_exp, aes(x = pH_treatment, y = ng_CH4_per_g_dry_soil_per_day_mean, fill = pH_treatment)) +
  geom_col(position = "dodge", color = "black") +
  geom_errorbar(aes(ymin = ng_CH4_per_g_dry_soil_per_day_mean - ng_CH4_per_g_dry_soil_per_day_se,
                    ymax = ng_CH4_per_g_dry_soil_per_day_mean + ng_CH4_per_g_dry_soil_per_day_se),
                width = 0.25, size = 0.3) +
...
ggsave(plot = pH_H2O_CH4_fluxes_means, filename = "figures/pH/fluxes/pH_AND_H2O_wetcontrol_CH4_fluxes_means.png", width = 8.5, height = 5)
