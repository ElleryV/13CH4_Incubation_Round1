  ...
  for (p in unique(data.plot$pH_treatment)) {
    ...
    #   ) +
      annotate(
        ...
        x = min(subset_data$CO2_ppm_inverse, na.rm = TRUE) + 0.35 * diff(range(subset_data$CO2_ppm_inverse, na.rm = TRUE)),
        y = min(subset_data$d13C_permil, na.rm = TRUE) + 0.9 * diff(range(subset_data$d13C_permil, na.rm = TRUE)),
        label = eqn,
        hjust = 0, vjust = 0,
        size = 5,
        color = "black",
    ...
    # Save plot to list with a descriptive name
    all_plots[[paste0(s, "_pH", p)]] <- p_plot
...
Making 4 paneled figures to include in supplemental information of the manuscript 

```{r 4 panel figure for pH exp - unamended soils}

# Then extract the 4 Unamended plots
plots_unamended <- all_plots[grepl("^Unamended_pH", names(all_plots))]

# Proceed with shared legend
# Shared legend
shared_legend <- get_legend(
  plots_unamended[[1]] + theme(legend.position = "bottom")
)

# Remove legends
plots_nolegend <- lapply(plots_unamended, function(p) p + theme(legend.position = "none"))

# 2x2 grid
panel_grid <- plot_grid(
  plotlist = plots_nolegend, 
  ncol = 2, 
  labels = "AUTO", # generates A, B, C, D for panel labels
  label_size = 20) # # increase number to make letters bigger

# Attach shared legend
composite_unamended <- plot_grid(panel_grid, shared_legend, ncol = 1, rel_heights = c(1, 0.12))

# Save
ggsave("figures/isotopes/keeling/pH_Composite_Unamended.png", composite_unamended, width = 15, height = 13)

```

```{r 4 panel figure for pH exp - ground rock soils}

# Then extract the 4 ground rock plots
plots_groundrock <- all_plots[grepl("^Ground Rock_pH", names(all_plots))]

# Proceed with shared legend
# Shared legend
shared_legend <- get_legend(
  plots_groundrock[[1]] + theme(legend.position = "bottom")
)

# Remove legends
plots_nolegend <- lapply(plots_groundrock, function(p) p + theme(legend.position = "none"))
...
# Attach shared legend
composite_groundrock <- plot_grid(panel_grid, shared_legend, ncol = 1, rel_heights = c(1, 0.12))

# Save
ggsave("figures/isotopes/keeling/pH_Composite_Ground Rock.png", composite_groundrock, width = 15, height = 13)
...
```{r NEW individual soil & pH treatment combo plots}
...
# Loop through combinations of soil type and pH treatment
for (s in unique(data.plot$soil_type)) {
  for (p in unique(data.plot$H2O_treatment)) {
    
    # Subset data
    subset_data <- data.plot %>%
      filter(soil_type == s, H2O_treatment == p)
    ...
    p_plot <- ggplot(subset_data, aes(x = CO2_ppm_inverse, y = d13C_permil, color = timepoint)) +
      ...
      stat_smooth(method = "lm", se = FALSE, color = "black") +
      labs(
        title = paste0("Keeling plot - ", "Soil: ", s, " H2O: ", p),
        y = "*&delta;*<sup>13</sup>C of CO<sub>2</sub> (&permil;)",
        x = "1/[CO<sub>2</sub>] (ppm<sup>-1</sup>)"
      ) +
      scale_color_manual(values=wes_palette("Darjeeling1", 2) ) +
    ...
    # Save plot to list with a descriptive name
    all_plots[[paste0(s, "_H2O", p)]] <- p_plot

    # Save each plot
    filename <- sprintf("figures/isotopes/keeling/Keeling_%s_H2O%s.png", s, p)
    ggsave(filename, p_plot, width = 9, height = 6.5)
  }
}
...
```{r OLD individual soil & H2O treatment combo plots}
...
```{r double checking H2O exp y-intercepts}

lm <- lm(d13C_permil ~ CO2_ppm_inverse,
         data = subset(data.plot, soil_type == "Unamended" & H2O_treatment == "no"))
summary(lm)
...
```{r 2 panel figure for H2O exp - unamended soils}

# Then extract the 2 Unamended plots
plots_unamended <- all_plots[grepl("^Unamended_H2O", names(all_plots))]
...
# Save
ggsave("figures/isotopes/keeling/H2O_Composite_Unamended.png", composite_unamended, width = 13, height = 7)

```

```{r 2 panel figure for H2O exp - ground rock soils}

# Then extract the 4 ground rock plots
plots_groundrock <- all_plots[grepl("^Ground Rock_H2O", names(all_plots))]
...
# Save
ggsave("figures/isotopes/keeling/H2O_Composite_Ground Rock.png", composite_groundrock, width = 13, height = 7)
