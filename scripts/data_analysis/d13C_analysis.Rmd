...
```{r preparing data to make pH T2-T1 difference figure}
...
```{r summarizing data for pH figure and for table for manuscript}
...
write_csv(summary_paper, "manuscript_materials/pH_d13C_summary.csv")

```

```{r pH exp T2-T1 difference stats & figure}
# 1) linear models
# Fit models for each soil_type and store results
pH_d13C_model <- pH_data %>% 
  ...
  set_names(unique(pH_data$soil_type))                   # name each list element by soil_type
...
```{r preparing data to make H2O T2-T1 difference figure}

# subsetting data for just H2O experiment 
H2O_data <- data %>%
  filter(
    experiment == "H2O",
    !timepoint %in% c("exp_zero", "final_dilcor")
  ) %>%
  rename(H2O_treatment = pH_treatment) %>%
  mutate(
    H2O_treatment = recode(H2O_treatment, "Wet control" = "yes"),
    H2O_treatment = fct_relevel(H2O_treatment, "no", "yes"),
    soil_type     = fct_relevel(soil_type, "Unamended", "Ground Rock")
  ) %>%
  dplyr::select(-jar_type, -experiment, -sample_id) %>%
  pivot_wider(names_from = timepoint,                    # pivoting data to calculate final - initial change in isotope values
              values_from = d13C_VPDB_perc) %>%
  mutate(d13C_diff = final-initial)

```

```{r summarizing data for H2O figure and for table for manuscript}
# summarizing data for figure 
H2O_summary <- H2O_data %>% 
...
# summarizing data for table in paper
summary_paper <- H2O_data %>% 
  pivot_longer(cols = c(initial, final),
               names_to = "variable",
               values_to = "value") %>% 
  group_by(soil_type, H2O_treatment, variable) %>% 
  summarise(
    mean = mean(value, na.rm = TRUE),
    sd = sd(value, na.rm = TRUE),
    n = dplyr::n(),
...
summary_paper <- summary_paper %>% 
  ...
  dplyr::select(soil_type:n,mean_initial,se_initial,mean_final,se_final)

write_csv(summary_paper, "manuscript_materials/H2O_d13C_summary.csv")

```

```{r H2O exp T2-T1 difference stats & figure}
